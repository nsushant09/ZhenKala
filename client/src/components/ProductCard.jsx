    ...
    >
      <div className="bg-[#FCF9EC] rounded-sm overflow-hidden hover:shadow-md transition-all duration-300 hover:scale-[1.02] w-full max-w-sm cursor-pointer group h-full flex flex-col">
        ...
        <div className="p-4 flex flex-col gap-2">
          {/* Product Name */}
          <h3 className="text-base text-gray-900 line-clamp-2 leading-tight min-h-[2.75rem]" title={name}>{name}</h3>
          ...
          <div className="flex items-baseline gap-2">
            ...
            {hasDiscount && (
              <span className="text-xs text-on-background/50 line-through decoration-1">
          ...
          <button
            className={`
              w-full mt-1 py-2.5 rounded-md text-sm transition-all duration-300 flex items-center justify-center gap-2 shadow-sm
              ${isAdding ? 'bg-secondary cursor-default' : 'bg-secondary hover:bg-red-500 hover:scale-[1.02]'} text-white
            `}
          ...
          >
            {isAdding ? (
              ...
              <><span className="text-lg font-light">+</span> <span className="font-medium">Add to Cart</span></>
            )}
          </button>
        </div>
      </div>
